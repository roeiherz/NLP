...
def get_q(tag_v, tag_w, tag_u, total_tokens, q_tri_counts, q_bi_counts, q_uni_counts, lambda1, lambda2):
    """
...
def hmm_viterbi(sent, total_tokens, q_tri_counts, q_bi_counts, q_uni_counts, e_word_tag_counts, e_tag_counts, lambda1, lambda2):
    ...
    for k in range(sent_length):
        for v in range(nof_tags):
            ...
            # calc e - and prune if equal to 0
            e_prob = get_e(sent[k], tag_v, e_word_tag_counts, q_uni_counts)
            if e_prob == 0.0:
                continue
            ...
            for u in range(nof_tags):
                ...
                for w in range(nof_tags):
                    ...
                    w_temp_val = phi_val * get_q(tag_v, tag_w, tag_u, total_tokens, q_tri_counts, q_bi_counts,
                                                 q_uni_counts, lambda1, lambda2) * e_prob
...
def lambda_grid_search():
    accuracies1 = []
    lambdas1 = []
    lambda1_list = [t/100.0 for t in range(45,65)]
    lambda2 = 0.3
    for lambda1 in lambda1_list:
        ...
        print("lambda1 - {0} - {1}".format(lambda1, acc_viterbi))
    ...
    lambda2_list = [t/100.0 for t in range(20,40)]
    accuracies2 = []
    lambdas2 = []
    for lambda2 in lambda2_list:
        ...
        print("lambda2 - {0} - {1}".format(lambda2, acc_viterbi))

    best_index = numpy.argmax(accuracies2)
    lambda2 = lambdas2[best_index]
    print("best lambda2 is {0}".format(lambda2))

    print("best accuracy is {0}".format(accuracies2[best_index]))


    plt.figure()
    plot_graph(accuracies2, lambdas2, "grid_search_lambda2", "accuracies2", "lambda2", "accuracy")
